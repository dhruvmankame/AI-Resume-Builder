...
const ResumePreview = forwardRef<HTMLDivElement, any>((_, ref) => {
  ...
  // Helper for LaTeX-style link rendering
  ...
  // Instant filtering logic for the live feed
  ...
  return (
    <div className="w-full flex flex-col h-full bg-white dark:bg-gray-900 transition-colors">
      ...
      {/* THE INSTANT FEED (Real-time Typography) */}
      <div className="flex-1 overflow-auto bg-gray-200 dark:bg-gray-950 flex justify-center p-0 transition-all duration-300">
        <div 
          ...
          style={{ 
            ...
            minHeight: "297mm",
            padding: "15mm 20mm",
            fontFamily: "'Computer Modern Serif', 'cmunrm', serif", // The exact LaTeX font
            lineHeight: "1.2"
          }}
        >
          {/* Header */}
          <div className="text-center mb-5">
            <h1 className="text-[24pt] font-normal uppercase mb-0.5 tracking-tight" style={{ fontVariant: "small-caps" }}>
            ...
            <div className="text-[10pt] flex flex-wrap justify-center gap-x-2">
              ...
              {personalInfo.linkedin && <span>| <a href={ensureProtocol(personalInfo.linkedin)} target="_blank" rel="noreferrer" className="text-blue-800 underline">LinkedIn</a></span>}
              {personalInfo.github && <span>| <a href={ensureProtocol(personalInfo.github)} target="_blank" rel="noreferrer" className="text-blue-800 underline">GitHub</a></span>}
              {customLinks?.map((link) => (
                <span key={link.id}>| <a href={ensureProtocol(link.url)} target="_blank" rel="noreferrer" className="text-blue-800 underline">{link.name}</a></span>
              ))}
            </div>
          </div>
              ...
              <div className="space-y-3">
                {validProjects.map((proj) => (
                  <div key={proj.id}>
                    ...
                    {proj.url && <div className="text-[9.5pt] italic text-blue-800 underline mb-0.5"><a href={ensureProtocol(proj.url)} target="_blank" rel="noreferrer">{proj.url}</a></div>}
                    <ul className="list-disc list-inside text-[10pt] pl-1 space-y-0.5">
