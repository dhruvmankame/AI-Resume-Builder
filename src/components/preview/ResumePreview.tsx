...
const ResumePreview = forwardRef<HTMLDivElement, any>((_, ref) => {
  ...
  // Re-implementing the 2-second debounce for high-quality LaTeX compilation
  useEffect(() => {
    setIsCompiling(true);
      ...
      <div className="w-full bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200 px-6 py-3 flex justify-between items-center shadow-sm z-10">
        ...
        <div className="flex gap-4">
          <button 
            ...
            className="flex items-center gap-2 text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400 transition-colors disabled:opacity-50 font-medium text-sm"
            title="Download PDF"
          >
      ...
      <div className="flex-1 relative overflow-hidden" ref={ref}>
        {pdfUrl ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-white dark:bg-gray-800 border-0">
             ...
             <p className="text-gray-500 dark:text-gray-400 text-sm font-medium animate-pulse">Initializing LaTeX compiler...</p>
          </div>
        )}
      </div>
    </div>
